...
class EquiposDemoSeeder extends Seeder
{
    public function run(): void
    {
        ...
        // ==================== CATEGORÍAS ====================
        // IMPORTANTE: El constraint único es sobre 'nombre' SOLO.
        // Por lo tanto usamos updateOrCreate con ['nombre'] como clave.
        // Si más adelante cambias el constraint a unique(['nombre','marca_id']),
        // cambia la clave de búsqueda a ['nombre' => ..., 'marca_id' => ...].
        ...
        foreach ($categoriasPorMarca as $nombreMarca => $categoriasLista) {
            ...
            foreach ($categoriasLista as $catData) {
                // ⚠️ CLAVE ÚNICA: solo 'nombre' porque el constraint es categorias_nombre_unique
                ...
                $categoriasCreadas[$catData['nombre']] = $categoria;
            }
        }
        $this->command->info('✅ ' . count($categoriasCreadas) . ' categorías únicas creadas/actualizadas');
        ...
        foreach ($modelosData as $mod) {
            // Buscar la categoría por nombre (único) y verificar que su marca coincida
            ...
            // Si la categoría existe pero su marca_id fue reasignado a otra marca
            // (porque el nombre es único global), igual la usamos, pero el modelo
            // llevará su propia marca_id.
            if (!$categoria) {
                // Fallback: buscar la categoría solo por nombre
