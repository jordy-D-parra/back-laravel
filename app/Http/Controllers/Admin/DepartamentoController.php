        ...
        if ($request->institucion_id && $request->usar_responsable_institucion && $request->responsable_id) {
            ...
            if ($responsable) {
                $responsable->update([
                    ...
                    'cargo' => $validated['representante_cargo'] ?? $responsable->cargo,
                ]);
            }
        } else {
            // Crear nuevo responsable para el departamento
            Responsable::create([
    ...
    public function update(Request $request, Departamento $departamento)
    {
        ...
        if ($request->institucion_id && $request->usar_responsable_institucion && $request->responsable_id) {
            ...
            Responsable::where('departamento_id', $departamento->id)
                ->where('id', '!=', $request->responsable_id)
                ->update(['departamento_id' => null]);

            // Asignar el responsable de la institución a este departamento
            $responsable = Responsable::find($request->responsable_id);
            if ($responsable) {
                $responsable->update([
                    'departamento_id' => $departamento->id,
            ...
            // Buscar responsable existente del departamento
