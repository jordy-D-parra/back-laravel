...
class SolicitudController extends Controller
{
    ...
    public function getDetalles($id)
    {
        try {
            ...
            // Obtener responsable desde la entidad
            $responsable = null;
            if ($solicitud->tipo_solicitante === 'interno' && $solicitud->departamento && $solicitud->departamento->responsable) {
                $responsable = $solicitud->departamento->responsable;
            } elseif ($solicitud->tipo_solicitante === 'externo' && $solicitud->institucion && $solicitud->institucion->responsable) {
                $responsable = $solicitud->institucion->responsable;
            }
    ...
    // ✅ MÉTODO CANCEL CORREGIDO - AHORA DEVUELVE JSON PARA AJAX
    ...
    // ✅ MÉTODO UPDATE CORREGIDO - TABLAS EN SINGULAR
    public function update(Request $request, $id)
    {
        \Log::info('===========================================');
        \Log::info('=== [UPDATE] MÉTODO UPDATE LLAMADO ===');
        \Log::info('===========================================');
        \Log::info('[UPDATE] ID solicitado: ' . $id);
        \Log::info('[UPDATE] Método HTTP: ' . $request->method());
        \Log::info('[UPDATE] URL completa: ' . $request->fullUrl());

        if ($request->has('_method')) {
            \Log::info('[UPDATE] _method detectado: ' . $request->get('_method'));
        } else {
            \Log::warning('[UPDATE] NO se encontró _method en la petición');
        }

        \Log::info('[UPDATE] Datos recibidos (raw):', $request->all());

        $user = auth()->user();
        \Log::info('[UPDATE] Usuario autenticado ID: ' . ($user ? $user->id : 'NO AUTENTICADO'));

        try {
            \Log::info('[UPDATE] Buscando solicitud con ID: ' . $id);
            $solicitud = Solicitud::find($id);

            if (!$solicitud) {
                \Log::error('[UPDATE] ❌ Solicitud NO encontrada con ID: ' . $id);
                return response()->json([
                    'success' => false,
            ...
            \Log::info('[UPDATE] ✅ Solicitud encontrada - Estado actual: ' . $solicitud->estado_solicitud);

            if ($solicitud->id_solicitante !== $user->id) {
                \Log::warning('[UPDATE] ⚠️ Usuario no autorizado');
                return response()->json([
                    'success' => false,
                    'message' => 'No autorizado para editar esta solicitud'
                ], 403);
            }

            if ($solicitud->estado_solicitud !== 'pendiente') {
                \Log::warning('[UPDATE] ⚠️ Intento de editar solicitud NO pendiente');
                return response()->json([
                    'success' => false,
            ...
            \Log::info('[UPDATE] Validando datos...');

            // ✅ CORREGIDO: Usar nombres de tablas en SINGULAR
            $validated = $request->validate([
                ...
                'institucion_id' => 'nullable|exists:institucion,id',
                'responsable_id' => 'nullable|exists:responsable,id',
            ]);

            \Log::info('[UPDATE] ✅ Datos validados correctamente:', $validated);

            \Log::info('[UPDATE] Actualizando solicitud en base de datos...');
            $solicitud->update($validated);

            \Log::info('[UPDATE] ✅ Solicitud actualizada exitosamente');
            \Log::info('===========================================');
            \Log::info('=== [UPDATE] ÉXITO - Solicitud ID: ' . $solicitud->id . ' ===');
            \Log::info('===========================================');
            ...
            \Log::error('[UPDATE] ❌ Error de validación:', $e->errors());
            return response()->json([
                'success' => false,
                'message' => 'Error de validación',
                'errors' => $e->errors()
            ...
            \Log::error('[UPDATE] Mensaje: ' . $e->getMessage());
            \Log::error('[UPDATE] Archivo: ' . $e->getFile() . ' línea: ' . $e->getLine());
