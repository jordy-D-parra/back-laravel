...
window.editarSolicitud = async function(id) {
    try {
        ...
        document.getElementById('editTipoSolicitante').value = data.tipo_solicitante;
        document.getElementById('editPrioridad').value = data.prioridad;
        if (data.fecha_requerida) document.getElementById('editFechaRequerida').value = data.fecha_requerida.split('T')[0];
        if (data.fecha_fin_estimada) document.getElementById('editFechaFin').value = data.fecha_fin_estimada.split('T')[0];
        document.getElementById('editJustificacion').value = data.justificacion || '';
        ...
        if (data.tipo_solicitante === 'interno') {
            ...
            if (data.departamento_id) document.getElementById('editDepartamentoId').value = data.departamento_id;
            if (data.departamento_id) await cargarResponsablePorDepartamentoEditar(data.departamento_id, data.responsable_id);
        } else {
            editInternoFields.style.display = 'none';
            editExternoFields.style.display = 'block';
            if (data.institucion_id) document.getElementById('editInstitucionId').value = data.institucion_id;
            if (data.institucion_id) await cargarResponsablePorInstitucionEditar(data.institucion_id, data.responsable_id);
        }
...
async function cargarResponsablePorDepartamentoEditar(departamentoId, responsableIdActual) {
    try {
        ...
        if (data.responsable) {
            display.innerHTML = `<strong>${escapeHtml(data.responsable.nombre)}</strong><br><small>${escapeHtml(data.responsable.cargo || '')} - ${escapeHtml(data.responsable.telefono || '')}</small>`;
...
window.confirmarEliminarSolicitud = function(id) {
    ...
    document.getElementById('deleteSolicitudNombre').textContent = `Solicitud #${id}`;
    new bootstrap.Modal(document.getElementById('modalEliminarSolicitud')).show();
};
...
async function eliminarSolicitud() {
    ...
    try {
        ...
        if (response.ok && result.success) {
            ...
            bootstrap.Modal.getInstance(document.getElementById('modalEliminarSolicitud')).hide();
            cargarPagina(currentPage);
        } else {
...
function initCrearEventListeners() {
    ...
    document.getElementById('departamentoSelect')?.addEventListener('change', function() {
        ...
        if (this.value === 'otro') {
            ...
            document.getElementById('responsableDisplay').innerHTML = '<span class="text-muted">Complete los datos del nuevo departamento</span>';
            document.getElementById('responsable_id_hidden').value = '';
        } else {
    ...
    document.getElementById('institucionSelect')?.addEventListener('change', function() {
        ...
        if (this.value === 'otro') {
            nuevoField.style.display = 'block';
            document.getElementById('responsableDisplay').innerHTML = '<span class="text-muted">Complete los datos de la nueva institución</span>';
            document.getElementById('responsable_id_hidden').value = '';
        } else {
            nuevoField.style.display = 'none';
    ...
    document.addEventListener('click', function(e) {
        ...
        if (e.target.classList.contains('remove-item-editar') || e.target.parentElement?.classList?.contains('remove-item-editar')) {
            const btn = e.target.classList.contains('remove-item-editar') ? e.target : e.target.parentElement;
            const card = btn.closest('.item-card');
    ...
    document.getElementById('add-item-editar')?.addEventListener('click', function() {
        const container = document.getElementById('items-container-editar');
...
document.getElementById('formEditarSolicitud')?.addEventListener('submit', async function(e) {
    ...
    let hasItems = false;
    for (let pair of formData.entries()) {
        if (pair[0].includes('item_descripcion') && pair[1] && pair[1].trim() !== '') {
            hasItems = true;
            break;
        }
    }
...
function initEventListeners() {
    ...
    limpiarBtn?.addEventListener('click', () => { searchInput && (searchInput.value = ''); estadoFilter && (estadoFilter.value = ''); prioridadFilter && (prioridadFilter.value = ''); aplicarFiltros(); });
    document.getElementById('btnConfirmarCancelar')?.addEventListener('click', window.confirmarCancelar);
    document.getElementById('btnConfirmarEliminarSolicitud')?.addEventListener('click', eliminarSolicitud);
}
