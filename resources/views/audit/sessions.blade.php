                    ...
                    <table class="table table-hover">
                        ...
                        <tbody>
                            @forelse($sessions as $session)
                              <tr>
                                <td>
                                    <strong>{{ $session->nombre ?? $session->name ?? 'Usuario' }} {{ $session->apellido ?? '' }}</strong><br>
                                    <small class="text-muted">Cédula: {{ $session->cedula ?? $session->user_cedula ?? 'N/A' }}</small>
                                </td>
                                <td>
                                    @php
                                        $user = \App\Models\User::find($session->user_id);
                                    @endphp
                                    {!! $user ? $user->role_badge : '<span class="badge bg-secondary">Sin rol</span>' !!}
                                </td>
