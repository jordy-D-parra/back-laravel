                ...
                <div class="card-body p-4" style="background: linear-gradient(135deg, #1e3c72 0%, #2a5298 100%); border-radius: 20px;">
                    <div class="d-flex justify-content-between align-items-center flex-wrap gap-3">
                        <div>
                            ...
                            <p class="text-white-50 mb-0">Clasifica los equipos por tipo (Laptop, Desktop, Monitor, etc.)</p>
                        </div>
            ...
            <div class="card shadow-sm border-0 rounded-4" style="background: white;">
                <div class="card-body p-4">
                    <div class="row g-3 align-items-end">
                        <div class="col-md-6">
                            <label class="form-label text-muted small">Buscar categoría</label>
                    ...
                    <div class="mt-3">
                        <span class="text-muted small">Mostrando <span id="resultadosCount">0</span> categorías</span>
                    </div>
                </div>
            </div>
        </div>
                ...
                <form id="formCrearCategoria">
                    <div class="row g-3">
                        <div class="col-12">
                            ...
                            <input type="text" name="nombre" id="nombreCategoria" class="form-control" placeholder="Ej: Laptop, Desktop, Monitor, Impresora" style="border-radius: 10px; background: white;" required>
                        </div>
                        <div class="col-12">
                            <label class="form-label fw-semibold">Descripción</label>
                            <textarea name="descripcion" id="descripcionCategoria" rows="3" class="form-control" placeholder="Descripción de la categoría" style="border-radius: 10px; background: white;"></textarea>
                        </div>
                        <div class="col-12">
                            <label class="form-label fw-semibold">Estado</label>
                            <select name="activo" id="activoCategoria" class="form-select" style="border-radius: 10px; background: white;">
                    ...
                    <div class="mt-4 pt-3 border-top d-flex justify-content-end gap-2">
                        ...
                        <button type="button" onclick="guardarCategoria()" class="btn px-4 text-white" style="background: linear-gradient(135deg, #1e3c72 0%, #2a5298 100%); border-radius: 10px;">Guardar Categoría</button>
                    </div>
                </form>
            </div>
        </div>
            ...
            <div class="modal-body p-4" style="background: white;">
                <form id="formEditarCategoria">
                    ...
                    <div class="mt-4 pt-3 border-top d-flex justify-content-end gap-2">
                        ...
                        <button type="button" onclick="actualizarCategoria()" class="btn px-4 text-white" style="background: linear-gradient(135deg, #1e3c72 0%, #2a5298 100%); border-radius: 10px;">Actualizar Categoría</button>
                    </div>
                </form>
            </div>
        </div>
...
<!-- MODAL VER MODELOS POR CATEGORÍA -->
...
let categorias = [
    { id: 1, nombre: 'Laptop', descripcion: 'Computadoras portátiles', activo: true, modelos: 12 },
    { id: 2, nombre: 'Desktop', descripcion: 'Computadoras de escritorio', activo: true, modelos: 8 },
    { id: 3, nombre: 'Monitor', descripcion: 'Pantallas y monitores', activo: true, modelos: 6 },
    { id: 4, nombre: 'Impresora', descripcion: 'Impresoras y multifuncionales', activo: true, modelos: 5 },
    { id: 5, nombre: 'Proyector', descripcion: 'Proyectores multimedia', activo: false, modelos: 2 },
    { id: 6, nombre: 'Tablet', descripcion: 'Tabletas digitales', activo: true, modelos: 4 },
    { id: 7, nombre: 'Servidor', descripcion: 'Servidores y racks', activo: true, modelos: 3 },
    { id: 8, nombre: 'Switch', descripcion: 'Switches de red', activo: true, modelos: 5 },
    { id: 9, nombre: 'Router', descripcion: 'Routers y puntos de acceso', activo: true, modelos: 4 },
    { id: 10, nombre: 'Almacenamiento', descripcion: 'Discos duros, SSD, NAS', activo: true, modelos: 7 }
];
...
function renderizarTabla() {
    ...
    const categoriasPagina = categoriasFiltradas.slice(start, end);
    const tbody = document.getElementById('tablaBody');
    const resultadosCount = document.getElementById('resultadosCount');

    resultadosCount.innerText = categoriasFiltradas.length;

    if (categoriasFiltradas.length === 0) {
        tbody.innerHTML = `<tr><td colspan="6" class="text-center py-5 text-muted" style="background: white;">No hay categorías registradas<br><button onclick="abrirModalCrear()" class="btn btn-sm mt-2" style="background: #1e3c72; color: white; border-radius: 8px;"> Registrar primera categoría</button></td></tr>`;
        document.getElementById('paginacionContainer').innerHTML = '';
        return;
    }

    let html = '';
    for (const c of categoriasPagina) {
        let estadoColor = c.activo ? '#1b5e20' : '#8b1a24';
        let estadoBg = c.activo ? '#e8f5e9' : '#ffebee';
        let estadoTexto = c.activo ? 'Activo' : 'Inactivo';

        html += `
            <tr>
                <td class="px-4 py-3 fw-semibold" style="color: #1a1a2e;">#${c.id}</td>
                <td class="px-4 py-3"><span class="badge-fecha" style="background: #e3f2fd; color: #0d47a1; font-weight: 600;">${escapeHtml(c.nombre)}</span></td>
                <td class="px-4 py-3" style="color: #495057;">${escapeHtml(c.descripcion || '-')}</td>
                <td class="px-4 py-3"><span class="badge-estado" style="background: ${estadoBg}; color: ${estadoColor};">${estadoTexto}</span></td>
                <td class="px-4 py-3 text-center"><button onclick="verModelos(${c.id})" class="btn btn-sm btn-accion-ver" style="border-radius: 8px; padding: 6px 12px;"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="2" y="4" width="20" height="16" rx="2"/><circle cx="12" cy="12" r="2"/></svg> ${c.modelos} modelos</button></td>
                <td class="px-4 py-3 text-center"><div class="d-flex gap-2 justify-content-center">
                    <button onclick="editarCategoria(${c.id})" class="btn btn-sm btn-accion-editar" style="border-radius: 8px; padding: 6px 12px;"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M20 14.66V20a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2h5.34"/><polygon points="18 2 22 6 12 16 8 16 8 12 18 2"/></svg> Editar</button>
                    <button onclick="eliminarCategoria(${c.id})" class="btn btn-sm btn-accion-eliminar" style="border-radius: 8px; padding: 6px 12px;"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M3 6h18M8 6V4h8v2M18 6v14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2V6"/></svg> Eliminar</button>
                </div></td>
            </tr>
        `;
    }
...
function renderizarPaginacion() {
    const totalPages = Math.ceil(categoriasFiltradas.length / itemsPorPagina);
...
function aplicarFiltros() {
    ...
    categoriasFiltradas = categorias.filter(c => {
        if (searchTerm && !c.nombre.toLowerCase().includes(searchTerm) && !(c.descripcion || '').toLowerCase().includes(searchTerm)) return false;
        if (estado) {
            const estadoBool = estado === 'activo';
            if (c.activo !== estadoBool) return false;
        }
        return true;
    });
    currentPage = 1;
...
function abrirModalCrear() { document.getElementById('formCrearCategoria').reset(); document.getElementById('modalCrear').style.display = 'flex'; }
function cerrarModalCrear() { document.getElementById('modalCrear').style.display = 'none'; }

function guardarCategoria() {
    const nombre = document.getElementById('nombreCategoria').value;
    if (!nombre) { alert('El nombre de la categoría es requerido'); return; }
    alert('Funcionalidad en desarrollo. La categoría se guardará en la base de datos próximamente.');
    cerrarModalCrear();
}
...
function editarCategoria(id) {
    ...
    if (categoria) {
        ...
        document.getElementById('editActivo').value = categoria.activo ? '1' : '0';
...
function actualizarCategoria() {
    alert('Funcionalidad en desarrollo. La categoría se actualizará en la base de datos próximamente.');
    cerrarModalEditar();
}

function eliminarCategoria(id) {
    if (confirm('¿Está seguro de que desea eliminar esta categoría?')) {
        alert('Funcionalidad en desarrollo. La categoría se eliminará de la base de datos próximamente.');
    }
}
...
function verModelos(id) {
    ...
    // Datos de ejemplo de modelos por categoría
    const modelosPorCategoria = {
        1: ['HP ProBook 450', 'Dell Latitude 3420', 'Lenovo ThinkPad T14', 'HP EliteBook 840', 'Dell XPS 13'],
        2: ['Dell OptiPlex 3080', 'HP Pavilion Desktop', 'Lenovo ThinkCentre', 'Apple Mac Mini'],
        3: ['Samsung Odyssey G7', 'Samsung T55', 'Dell UltraSharp', 'LG 27UK850'],
        4: ['Epson EcoTank L3150', 'HP LaserJet Pro', 'Brother MFC', 'Canon PIXMA'],
        5: ['Epson EB-2155W', 'BenQ MH733', 'Optoma HD146X'],
        6: ['iPad Air 5ta Gen', 'Samsung Galaxy Tab S8', 'Microsoft Surface Pro'],
        7: ['Dell PowerEdge', 'HP ProLiant', 'Lenovo ThinkSystem'],
        8: ['Cisco Catalyst', 'TP-Link SG1024', 'Netgear GS308'],
        9: ['MikroTik hAP ac2', 'Ubiquiti UniFi', 'TP-Link Archer'],
        10: ['Samsung 870 EVO', 'WD Blue', 'Seagate Barracuda', 'Kingston A2000']
    };

    const modelos = modelosPorCategoria[id] || [];
    ...
    if (modelos.length === 0) {
        modalBody.innerHTML = `
            <div class="text-center py-5">
                ...
                <p class="mt-3 text-muted">Esta categoría no tiene modelos asociados</p>
        ...
        let html = `
            <div class="mb-3 pb-2 border-bottom">
                <strong class="text-primary">${escapeHtml(categoria.nombre)}</strong> - ${modelos.length} modelos
            </div>
            <div class="table-responsive">
                <table class="table table-sm">
                    <thead>
                        <tr><th>Modelo</th><th>Marca</th><th>Estado</th></thead>
                    </thead>
                    <tbody>
        `;
        for (const modelo of modelos) {
            let marca = modelo.split(' ')[0];
            html += `<tr>
                <td><span class="badge-fecha" style="background: #e3f2fd; color: #0d47a1; font-weight: 600;">${escapeHtml(modelo)}</span></td>
                <td class="text-muted">${escapeHtml(marca)}</td>
                <td><span class="badge-estado" style="background: #e8f5e9; color: #1b5e20;">Activo</span></td>
            </td>`;
        }
        html += `</tbody></tr></div>`;
