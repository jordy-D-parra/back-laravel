...
use App\Http\Controllers\Seriales_ActivoController;
...
Route::middleware(['auth'])->prefix('inventario')->name('inventario.')->group(function () {
    // Rutas para super_admin y admin
    Route::middleware(['role:super_admin,admin'])->group(function () {
        ...
        // NUEVAS RUTAS para componentes y funcionalidades extendidas
        Route::get('/componentes-por-categoria/{categoriaId}', [ActivoController::class, 'componentesPorCategoria'])
            ->name('componentes.categoria');
        Route::get('/activos-disponibles', [ActivoController::class, 'getActivosDisponibles'])
            ->name('activos.disponibles');
        Route::get('/estadisticas', [ActivoController::class, 'getEstadisticas'])
            ->name('estadisticas');
        Route::get('/reporte-capacidad', [ActivoController::class, 'reportePorCapacidad'])
            ->name('reporte.capacidad');
        Route::get('/reporte-categoria', [ActivoController::class, 'reportePorCategoria'])
            ->name('reporte.categoria');
        
        // ========== NUEVAS RUTAS PARA SERIALES ==========
        Route::prefix('seriales')->name('seriales.')->group(function () {
            Route::get('/', [Seriales_ActivoController::class, 'index'])->name('index');
            Route::get('/disponibles', [Seriales_ActivoController::class, 'getSerialesDisponibles'])->name('disponibles');
            Route::get('/activo/{activoId}', [Seriales_ActivoController::class, 'getSerialesByActivo'])->name('por-activo');
            Route::get('/buscar', [Seriales_ActivoController::class, 'buscar'])->name('buscar');
            Route::get('/reporte-estado', [Seriales_ActivoController::class, 'reportePorEstado'])->name('reporte');
            Route::get('/historial/{id}', [Seriales_ActivoController::class, 'historial'])->name('historial');
            Route::post('/{id}/asignar', [Seriales_ActivoController::class, 'asignar'])->name('asignar');
            Route::put('/{id}/estado', [Seriales_ActivoController::class, 'cambiarEstado'])->name('cambiar-estado');
            Route::post('/{id}/reparacion', [Seriales_ActivoController::class, 'marcarEnReparacion'])->name('reparacion');
            Route::post('/{id}/liberar', [Seriales_ActivoController::class, 'liberar'])->name('liberar');
            Route::post('/{id}/baja', [Seriales_ActivoController::class, 'darBaja'])->name('baja');
            Route::delete('/{id}', [Seriales_ActivoController::class, 'destroy'])->name('destroy');
        });
    });
    
    // Rutas de solo lectura para todos los usuarios autenticados
    Route::get('/ver/{id}', [ActivoController::class, 'show'])->name('ver');
    Route::get('/componentes/listar/{categoriaId}', [ActivoController::class, 'componentesPorCategoria'])
...
Route::middleware(['auth'])->prefix('soporte')->name('soporte.')->group(function () {
    ...
    Route::get('/soporte/{ficha}/datos', [SoporteController::class, 'getDatosFicha'])->name('soporte.datos');
